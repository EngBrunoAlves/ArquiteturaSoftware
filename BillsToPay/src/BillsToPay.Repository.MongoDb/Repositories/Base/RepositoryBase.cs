...
using BillsToPay.Repository.MongoDb.EntityConfig;
...
{
	internal abstract class RepositoryBase<TEntity> : IRepositoryBase<TEntity> where TEntity : IEntityBase
	{
		...
		public async Task<IEnumerable<TEntity>> Find(Expression<Func<TEntity, bool>> predicate)
		{
			...
			{
				return Collection.FindAsync(predicate);
			});
		...
		public async Task<IEnumerable<TEntity>> Find(Expression<Func<TEntity, bool>> predicate, int pageIndex, int size)
		{
			...
			return (await result.ToListAsync()).Skip(pageIndex).Take(size);
		}
		...
		public async Task<IEnumerable<TEntity>> Find(Expression<Func<TEntity, bool>> predicate, Expression<Func<TEntity, object>> order, int pageIndex, int size)
		{
			...
			return result.ToList().Skip(pageIndex).Take(size);
		}

		public async Task<IEnumerable<TEntity>> Find(Expression<Func<TEntity, bool>> filter, Expression<Func<TEntity, object>> order, int pageIndex, int size, bool isDescending)
		{
